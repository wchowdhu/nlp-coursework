...
import sys, codecs, optparse, os, csv, operator, math
...
for line in list:
    line.append(math.log(Pw.__call__(line[0]), 2))
...
with open(opts.input) as f:
  ...
  for line in f:
      ...
      if oneword[0] not in heap:
          heap.append(Entry(oneword[0], 0, math.log(Pw.__call__(oneword[0]),2), None))
      while len(heap)!=0:
          objentry = heap[0]
          word = heap[0].word
          prob = heap[0].logprob
          ...
          if newindex <= len(oneword)-1 and oneword[newindex] not in heap:
              heap.append(Entry(oneword[newindex], newindex, math.log(Pw.__call__(oneword[newindex]),2), objentry))
