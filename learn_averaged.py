...
def computePRO(nbests,theta):
  theta_cache = [float(0.0) for _ in xrange(len(theta))]
  T = float(len(nbests))
  step = epochs * T
  # apply averaged perceptron
    ...
    for nbest in nbests:
      ...
      for tup in sample:
        ...
        if sum([x * y for x, y in zip(theta, s1.features)]) <= sum([x * y for x, y in zip(theta, s2.features)]):
          ...
          update_cache = [item * eta * (step / (epochs * T)) for item in [x - y for x, y in zip(s1.features, s2.features)]]
          # theta = theta + update
          theta = [x + y for x, y in zip(theta, update)]
          theta_cache = [x + y for x, y in zip(theta_cache, update_cache)]
