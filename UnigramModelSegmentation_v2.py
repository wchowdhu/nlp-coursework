...
def findlogp(word):
    ...
    if count == None:
        if word.isdigit():
            ...
            count = 0.001
        elif re.match(u'[\d]?[\d][月] [\d]?[\d][日] (\d+)[年]', word) is not None:
            # print re.match(u'[\d]?[\d][月] [\d]?[\d][日] (\d+)[年]', word).group()
            count = 1
        elif re.match(u'(\d+)[年] [\d]?[\d][月]', word) is not None:
            # print re.match(u'(\d+)[年] [\d]?[\d][月]', word).group()
            count = 1
        elif re.match(u'[\d]?[\d][月] [\d]?[\d][日]', word) is not None:
            # re.match(u'[\d]?[\d][月] [\d]?[\d][日]', word).group()
            count = 1
        elif re.match(u'(\d+)[年]', word) is not None:
            # re.match(u'(\d+)[年]', word).group()
            count = 1
	elif len(word)<=3:
            count = 0.00000009
        else:
